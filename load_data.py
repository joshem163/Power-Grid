...
def process_data(N_scenarios):
    G=nx.read_gml("8500busEx.gml") # read from the path of the .gml file
...
path='./data' # datasett path
def load_data_outage(bus_name,N_scenarios):
    base_path = os.path.join(path,bus_name)
    graph_path = os.path.join(base_path, f"{bus_name}.gml")
    pickle_path = os.path.join(base_path, f'PartialObservdata_{bus_name}.pkl')
